...
from multi_tenant.utils import get_tenant_model
...
class TenantSyncRouter(object):
    ...
    def allow_migrate(self, db, app_label, model_name=None, **hints):
        ...
        from multi_tenant.utils import get_public_schema_name, app_labels
        from multi_tenant.postgresql_backend.base import DatabaseWrapper as TenantDbWrapper

        db_engine = settings.DATABASES[db]['ENGINE']
        if not (db_engine in ['tenant_schemas.postgresql_backend', 'multi_tenant.postgresql_backend'] or
        ...
        if connection.schema_name == get_public_schema_name():
            ...
            if isinstance(connection.tenant, get_tenant_model()):
                if connection.tenant.is_leading():
                    ...
                    if app_label not in app_labels(settings.TENANT_DETAIL_APPS):
                        return False
